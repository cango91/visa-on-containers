...
export async function connectRabbitMQ() {
  ...
  const { GATEWAY_RABBIT_USER, GATEWAY_RABBIT_PASSWORD } = process.env;
  fullConnString = [fullConnString[0], "://", GATEWAY_RABBIT_USER!, ":", GATEWAY_RABBIT_PASSWORD!, '@', fullConnString[1]];
...
export async function res() {
  try {
    ...
    await channel.assertExchange('gateway-exchange', 'direct', { durable: true, });
  } catch (error) {
